...
const PROV={
  gemini:{
    ...
    models:['gemini-3.8-flash','gemini-3.1-flash-lite','gemini-3.1-pro'],
...
async function callAI(msgs,sys){
  ...
  try{
    ...
    if(!res.ok){
      ...
      if(res.status===401||res.status===403) throw new Error('The provider rejected the key. Check the key, model access, billing/free-tier status, and browser restrictions. '+m);
...
function build(){
  ...
  $('aiSave').onclick=()=>{readForm();persist();msg('Saved ✓ — connection settings are stored only in this browser.');paintStatus();};
